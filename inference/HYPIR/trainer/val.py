...
class BaseTrainer:
    ...
    def validate(self):
        ...
        val_save_dir = os.path.join(self.config.output_dir, self.config.logging_dir, "val_images", f"{self.global_step:06}")
        if self.accelerator.is_main_process:
            os.makedirs(val_save_dir, exist_ok=True)
            os.makedirs(os.path.join(val_save_dir, 'compare'), exist_ok=True)
            os.makedirs(os.path.join(val_save_dir, 'pred'), exist_ok=True)
        ...
        with torch.no_grad():
            for i, batch in enumerate(pbar_val):
                ...
                lq_img = (self.batch_inputs.lq + 1) / 2
                ...
                if self.accelerator.is_main_process and i < 204: # Save first 5 batches
                    ...
                    # x shape: [B, C, H, W]
                    vis_imgs = torch.cat([gt_img, lq_img, pred_img], dim=3) # Concat horizontally
                    
                    image_arrs = (vis_imgs * 255.0).clamp(0, 255).to(torch.uint8) \
                        .permute(0, 2, 3, 1).contiguous().cpu().numpy()

                    pred_arr = (pred_img * 255.0).clamp(0, 255).to(torch.uint8) \
                        .permute(0, 2, 3, 1).contiguous().cpu().numpy()
                    
                    for j, img in enumerate(image_arrs):
                        filename = self.batch_inputs.filename
                        if filename == "":
                            filename = f"batch{i}_sample{j}_gt_vs_pred.png"

                        print(f"{filename=}")
                        Image.fromarray(img).save(os.path.join(val_save_dir, 'compare', filename[0]))
                    
                    for j, img in enumerate(pred_arr):
                        filename = self.batch_inputs.filename
                        if filename == "":
                            filename = f"batch{i}_sample{j}_pred.png"

                        print(f"{filename=}")
                        Image.fromarray(img).save(os.path.join(val_save_dir, 'pred', filename[0]))
    ...
    def optimize_generator_image(self):
        ...
        with ctx:
            ...
            loss_G = loss_l2 + loss_lpips + loss_disc + proj_loss + loss_edge + loss_tv + loss_l1 + loss_fdl
            self.accelerator.backward(loss_G)
            if self.accelerator.sync_gradients:
        ...
        # Log something
        loss_dict = dict(G_total=loss_G, G_mse=loss_l2, G_l1=loss_l1, G_lpips=loss_lpips, G_disc=loss_disc, G_repa=proj_loss, G_edge=loss_edge, G_tv=loss_tv, G_fdl=loss_fdl)
    ...
    #     if self.config.use_D:
    #         self.unwrap_model(self.D).eval().requires_grad_(False)
    #     x, zs_pred = self.forward_generator()
    #     x = x.float()
    #     loss_l2 = F.mse_loss(x, self.batch_inputs.z_gt.float(), reduction="mean") * self.config.lambda_l2
    #     # if self.config.use_repa:
    #     #     loss_repa = self.repa_loss(zs=self.batch_inputs.z_s, zs_pred=zs_pred)
    #     if self.config.use_vae:
    #         loss_lpips = self.net_lpips(x, self.batch_inputs.gt).mean() * self.config.lambda_lpips
    ...
    #                 proj_loss += torch.mean(-(z_j * z_pred_j).sum(dim=-1))
    #         proj_loss /= (len(zs) * bsz)
            
    #         # 确保 repa_loss 也是 float32
    #         loss_repa_float = proj_loss.float()
    #         losses = [("l2", loss_l2), ("lrepa", loss_repa_float)]
    #     grad_dict = {}
    #     self.G_opt.zero_grad()
