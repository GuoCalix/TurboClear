...
def wavelet_blur_np(image: np.ndarray, radius: int):
    ...
    for c in range(image.shape[0]):
        orig_shape = image[c].shape
        blurred_c = convolve(image[c], kernel, mode='nearest')
        if radius > 1:
            downsampled = zoom(blurred_c, 1 / radius, order=1)
            # Compute exact zoom factors to restore original shape
            zoom_factors = tuple(o / d for o, d in zip(orig_shape, downsampled.shape))
            blurred_c = zoom(downsampled, zoom_factors, order=1)
        blurred[c] = blurred_c
    return blurred
...
def attention_guided_fusion(
    ...
    _, attn_map = cv2.threshold(attn_map, threshold_value, 255, cv2.THRESH_BINARY)
