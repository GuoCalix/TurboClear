...
def safe_init_projector(module, scale=0.01):
    """
...
class ProjectionLayer(nn.Module):
    ...
    def build_projection_layer(self, projection_layer_type, **kwargs):
        ...
        self.projection_layer.apply(safe_init_projector)
        print("specially init projection layer")
...
class ZImageTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        projector_dim = dim * 2
        self.projectors = ProjectionLayer(
            projection_layer_type="conv", hidden_size=dim,
            projector_dim=dim*2, z_dim=z_dim, proj_kwargs_kernel_size=3
        )
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            for i, layer in enumerate(self.layers):
                ...
                if (i + 1) == self.encoder_depth:
                    # print(f"{unified.max()=}")
                    print(f"Is unified finite: {torch.isfinite(unified).all()}")
                    zs = self.projectors(unified[:, :4096, :])
        else:
            for i, layer in enumerate(self.layers):
    ...
    def from_pretrained_local(cls, pretrained_model_path, subfolder=None, **kwargs):
        ...
        if os.path.isfile(model_file):
            ...
            import glob
            sharded_files = sorted(glob.glob(os.path.join(pretrained_model_path, "diffusion_pytorch_model*.safetensors")))
            if not sharded_files:
                raise RuntimeError(f"No model weights found in {pretrained_model_path}")
            
            print(f"Loading weights from {len(sharded_files)} files...")
            for file_path in sharded_files:
                shard_state_dict = safetensors.torch.load_file(file_path, device="cpu")
                state_dict.update(shard_state_dict)

        # --- 处理 x_embedder 通道数扩展 (例如 16 -> 32) ---
        for key in list(state_dict.keys()):
            if "all_x_embedder" in key and "weight" in key:
                ckpt_weight = state_dict[key]
                if key not in model.state_dict(): continue
                model_weight = model.state_dict()[key]
                
                if ckpt_weight.shape != model_weight.shape and ckpt_weight.shape[0] == model_weight.shape[0]:
                    try:
                        patch_info = key.split('.')[1] 
                        p_size, fp_size = map(int, patch_info.split('-'))
                        pixels_per_patch = p_size * p_size * fp_size
                        
                        old_in_c = ckpt_weight.shape[1] // pixels_per_patch
                        new_in_c = model_weight.shape[1] // pixels_per_patch
                        
                        if new_in_c > old_in_c:
                            print(f"Expanding {key} input channels: {old_in_c} -> {new_in_c}")
                            w_old = ckpt_weight.view(ckpt_weight.shape[0], pixels_per_patch, old_in_c)
                            w_new = torch.zeros((ckpt_weight.shape[0], pixels_per_patch, new_in_c), 
                                              dtype=ckpt_weight.dtype, device=ckpt_weight.device)
                            w_new[:, :, :old_in_c] = w_old
                            state_dict[key] = w_new.view(model_weight.shape)
                    except Exception as e:
                        print(f"Warning: Failed to auto-adjust shape for {key}: {e}")
