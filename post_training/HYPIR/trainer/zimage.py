...
class ZImageTrainer(BaseTrainer):
    ...
    def prepare_batch_inputs(self, batch, transform=None):
        ...
        # z_null = torch.zeros((bs, 16, 128, 128)).to(device=self.device, dtype=self.weight_dtype)
        z_null = torch.load("debug_inputs/feature_null.pt").to(device=self.device, dtype=self.weight_dtype)
    ...
    def forward_generator(self):
        ...
        model_out_list = self._denoise_step(
            # noised_latent_model_input, t_expand,
        ...
        latents = self.step(latent_model_input, noise_pred, sigmas, 0) - self.batch_inputs.z_null
    ...
    def forward_generator_2_steps(self):
        latent_model_input = self.batch_inputs.z_lq
        timesteps = [1000, 750, 0]
        sigmas = (torch.tensor(timesteps) / 1000.0).to(dtype=torch.float32, device=self.device)
        bs = latent_model_input.shape[0]
        for i in range(2):
            timestep = torch.full((bs,), timesteps[i], dtype=torch.long, device=self.device)
            t_expand = (1000 - timestep) / 1000
            model_out_list = self._denoise_step(
                latent_model_input, t_expand, 
                self.c_txt["prompt_embeds"],
                timesteps_r=None
            ...
            latents = self.step(latent_model_input, noise_pred, sigmas, i)
            latent_model_input = latents.to(self.weight_dtype)
    ...
    #         if getattr(self.config, "use_D", False) and hasattr(self, "D") and self.D is not None:
    #             rand_t = random.choice([0, 250, 500, 750, 1000])
    #             bs = latents.shape[0]
    #             rand_timesteps = torch.full((bs,), rand_t, dtype=torch.long, device=self.device)
    #             # In case of the nuymerical gap between z_gt and latents
    #             latents = (latents / self.vae.config.scaling_factor) + self.vae.config.shift_factor
    #             _, fake_logits = D_unwrapped(latents, rand_timesteps, self.c_txt["prompt_embeds"])
    #             with torch.no_grad():
    #                 _, real_logits = D_unwrapped(self.batch_inputs.z_gt, rand_timesteps, self.c_txt["prompt_embeds"])
    ...
    #         bs = latents.shape[0]
    #         rand_timesteps = torch.full((bs,), rand_t, dtype=torch.long, device=self.device)
    #         # numerical gap between z_gt and latents
    #         latents = (latents / self.vae.config.scaling_factor) + self.vae.config.shift_factor
    #         sigma = rand_t / 1000.0
    #         epsilon = torch.rand_like(latents)
    #         noised_latents = (1 - sigma) * latents + sigma * epsilon
    #         noised_gt = (1 - sigma) * gt + sigma * epsilon
    #         _, real_logits = self.D(noised_gt, rand_timesteps, self.c_txt["prompt_embeds"])
    #         _, fake_logits = self.D(noised_latents, rand_timesteps, self.c_txt["prompt_embeds"])
    ...
    #             noised_gt = gt + noise
    #             _, real_logits_noisy = self.D(noised_gt, rand_timesteps, self.c_txt["prompt_embeds"])
