...
from HYPIR.trainer.val import BaseTrainer, BatchInput
...
class ZImageTrainerVal(BaseTrainer):
    ...
    #         latents = (latents / self.vae.config.scaling_factor) + self.vae.config.shift_factor
    #         _, real_logits = self.D(gt, rand_timesteps, self.c_txt["prompt_embeds"])
    #         _, fake_logits = self.D(latents, rand_timesteps, self.c_txt["prompt_embeds"])
