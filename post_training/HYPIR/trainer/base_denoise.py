...
class BaseTrainer:
    ...
    def init_lr_schedulers(self):
        ...
        logger.info(f"Creating {self.config.lr_scheduler_type} schedulers")
        lr_scheduler = getattr(self.config, "lr_scheduler", "constant")
    ...
    def init_optimizers(self):
        ...
        logger.info(f"Whether has projector: {hasattr(self, 'projector')}")

        if getattr(self.config, "use_qwen", False) and hasattr(self, 'projector'):
            logger.info("Add projector weights to optimizer")
            projector_params = self.projector.parameters()
            self.G_params.extend(projector_params)
        
        if getattr(self.config, "use_denoiser", True) and hasattr(self, "denoiser"):
            logger.info("Add denoiser weights to optimizer")
            # denoiser_params = list(filter(lambda p: p.requires_grad, self.denoiser.parameters()))
            denoiser_params = self.denoiser.parameters()
            self.G_params.extend(denoiser_params)
    ...
    def _save_config(self):
        ...
        try:
            ...
            if dataclasses.is_dataclass(self.config):
                cfg_obj = dataclasses.asdict(self.config)
            elif hasattr(self.config, "to_dict"):
                cfg_obj = self.config.to_dict()
            else:
                try:
                    cfg_obj = dict(self.config)
                except Exception:
                    cfg_obj = getattr(self.config, "__dict__", {})
        except Exception:
            cfg_obj = getattr(self.config, "__dict__", {})
    ...
    def optimize_generator_image(self):
        ...
        with ctx:
            ...
            self.G_pred = x
            loss_l2 = F.mse_loss(latents, self.batch_inputs.z_gt, reduction="mean") * self.config.lambda_l2
            # loss_l2 = torch.zeros((), device=self.device, dtype=self.weight_dtype)
            loss_l1_latent = F.l1_loss(latents, self.batch_inputs.z_gt, reduction="mean") * 0.2
            loss_l1 = F.l1_loss(x, self.batch_inputs.gt, reduction="mean") * self.config.lambda_l1
            cosine_sim = F.cosine_similarity(latents, self.batch_inputs.z_gt, dim=1)
            cosine_similarity_loss = 1 - cosine_sim.mean()
            # logger.info(f"loss_l1 dtype: {loss_l1.dtype}")
            ...
            if self.config.use_fdl:
                fdl_loss_fn = FDL_loss().to(self.device)
                loss_fdl = fdl_loss_fn(x, self.batch_inputs.gt) * self.config.lambda_fdl
            else:
            ...
            if getattr(self.config, "use_D", False) and hasattr(self, "D") and self.D is not None and not self.is_warmup:
                ...
                # print(f"gt dtype: {self.batch_inputs.gt.dtype}")
            ...
            loss_G = loss_l2 + loss_lpips + loss_dists +loss_disc + proj_loss + loss_edge + loss_tv + loss_l1 + loss_fdl + cosine_similarity_loss + loss_l1_latent
            self.accelerator.backward(loss_G)
            if self.accelerator.sync_gradients:
        ...
        # Log something
        loss_dict = dict(G_total=loss_G, G_mse=loss_l2, G_l1=loss_l1, G_latent_l1=loss_l1_latent, G_lpips=loss_lpips, G_fdl=loss_fdl, G_dists=loss_dists, G_disc=loss_disc, G_sim=cosine_similarity_loss)
    ...
    def optimize_discriminator(self):
        ...
        with ctx:
            ...
            # normalized_gt = Normalize(CLIP_DEFAULT_MEAN, CLIP_DEFAULT_STD)(gt)
            # normalized_x = Normalize(CLIP_DEFAULT_MEAN, CLIP_DEFAULT_STD)(x)
            ...
            if self.config.use_r1:    
                ...
                approx_r1 = torch.tensor(0.0, device=self.device, dtype=self.weight_type)
            # print(f"just Debug, approx_r1: {approx_r1}")
            # loss_D = loss_D_real.mean() + loss_D_fake.mean() + approx_r1
    ...
    def _save_minimal_checkpoint(self, save_path: str):
        ...
        # Save only trainable parameters (e.g., LoRA) to state_dict.pth and a small trainer_state.json
        ...
        torch.save(state_dict, os.path.join(save_path, "state_dict.pth"))
    ...
    def _load_minimal_checkpoint(self, load_path: str):
        # Load LoRA/trainable parameters from state_dict.pth if present
        state_file = os.path.join(load_path, "state_dict.pth")
        safetensor_file = os.path.join(load_path, "model.safetensors")
        if not os.path.exists(state_file):
            logger.warning(f"Minimal checkpoint missing: {state_file}")
            return
        model = self.unwrap_model(self.G)
        ...
        # if missing:
        #     logger.info(f"Minimal resume missing keys: {missing}")
        # if unexpected:
        #     logger.info(f"Minimal resume unexpected keys: {unexpected}")
        state_dict = torch.load(os.path.join(load_path, "state_dict.pth"), map_location="cpu")
        load_result = model.load_state_dict(state_dict, strict=False)
        missing = getattr(load_result, "missing_keys", [])
        unexpected = getattr(load_result, "unexpected_keys", [])
        ...
        if getattr(self.config, "use_qwen") and hasattr(self, "projector"):
            proj_path = os.path.join(load_path, "projector.pth")
            if os.path.exists(proj_path):
                proj_state_dict = torch.load(proj_path, map_location="cpu")
                projector = self.unwrap_model(self.projector)
                load_result = projector.load_state_dict(proj_state_dict, strict=False)

        if getattr(self.config, "use_denoiser") and hasattr(self, "denoiser"):
            denoise_path = os.path.join(load_path, "denoiser.pth")
            if os.path.exists(denoise_path):
                denoise_state_dict = torch.load(denoise_path, map_location="cpu")
                denoiser = self.unwrap_model(self.denoiser)
                load_result = denoiser.load_state_dict(denoise_state_dict, strict=False)