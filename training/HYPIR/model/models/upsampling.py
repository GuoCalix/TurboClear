...
class Upsample2D(nn.Module):
    ...
    def __init__(
        ...
        if use_conv_transpose:
            ...
            factor = 4
            conv = nn.Conv2d(self.channels, self.out_channels * factor, kernel_size=3, padding=1, bias=bias)
            self.repeats = factor * self.out_channels // self.channels
            self.smoothing_conv = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1)
    ...
    def forward(self, hidden_states: torch.Tensor, output_size: Optional[int] = None, *args, **kwargs) -> torch.Tensor:
        ...
        if self.use_pixel_shuffle:
            if self.name == "conv":
                h = self.conv(hidden_states)
            else:
                h = self.Conv2d_0(hidden_states)
            h = rearrange(h, "b (r1 r2 c) h w -> b c (h r1) (w r2)", r1=2, r2=2)
            h = self.smoothing_conv(h)
