...
def spectral_penalty(latent, periods_lat=(2,), bandwidth=0.15, weight=1.0, anisotropic=False, savepath=None, idx=None):
    ...
    if savepath is not None:
        ...
        save_latent_features(mask, savepath, f"mask_")
        save_latent_features(P, savepath, f"P_", scale_factor=4)
        save_latent_features(P * mask, savepath, f"maskP_", scale_factor=4)
...
def save_latent_features(latent: torch.Tensor, out_dir="output/features", savenames="", scale_factor: int = 1, step: int = 3):
    """
    将 latent 可视化并保存到文件。
    ...
    Args:
        ...
        out_dir: 保存目录
        savenames: 文件名前缀
        scale_factor: 输出缩放倍数 (例如 4 表示放大到 4x)
        mode: 可视化模式。
              - '3_channel_groups': 每 3 个通道用 PCA/RGB 合成为一张图。
              - '1_channel_per_image': 每个通道单独保存为灰度图。
    """
    if latent.ndim == 4:
        assert latent.size(0) == 1, "latent 形状应为 [1, C, H, W]"
        ...
        assert False, f"latent 形状 {latent.shape=} 不支持"

    os.makedirs(out_dir, exist_ok=True)
    x = latent.to(torch.float32).detach().cpu()  # [C, H, W]

    # 按步长分组处理通道
    for start in range(0, C, step):
        end = min(start + step, C)
        group = x[start:end]  # [g, H, W], g=1/2/3

        # --- 可视化处理 ---
        if group.shape[0] == 3:
            # 3个通道，作为RGB
            img = group.numpy().transpose(1, 2, 0)
            ...
            img = ch
        else: # group.shape[0] == 2
            # PCA 降维到 3 维（即使只有2维）
            flat = group.view(group.shape[0], -1).T.numpy()
            pca_dims = min(group.shape[0], 3)
            rgb = PCA(n_components=pca_dims).fit_transform(flat)
            if pca_dims < 3:
                # 补齐到3维
                rgb_full = np.zeros((rgb.shape[0], 3))
                rgb_full[:, :pca_dims] = rgb
                rgb = rgb_full
        ...
        # --- 缩放 ---
        if scale_factor != 1:
            tensor_img = torch.tensor(img).permute(2, 0, 1).unsqueeze(0) if img.ndim == 3 else torch.tensor(img)[None, None]
            up_img = F.interpolate(tensor_img, scale_factor=scale_factor, mode='bilinear', align_corners=False)
            if img.ndim == 3:
                img = up_img[0].permute(1, 2, 0).numpy()
            else:
                img = up_img[0, 0].numpy()

        # --- 保存 ---
        if step == 1:
            save_path = os.path.join(out_dir, f"{savenames}c{start}.png")
        else:
            save_path = os.path.join(out_dir, f"{savenames}c{start}-{end-1}.png")
