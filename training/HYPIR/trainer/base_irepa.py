...
class BaseTrainer:
    ...
    def init_dists(self):
        self.metric_dist = pyiqa.create_metric('dists', device=self.device)
    ...
    def repa_loss(self, zs, zs_pred):
        from iREPA.ldm.projection_loss import make_projection_loss
        loss_fn = make_projection_loss('cosine')
        proj_loss = 0.
        bsz = zs[0].shape[0]
        for i, (z, z_pred) in enumerate(zip(zs, zs_pred )):
            proj_loss  = proj_loss + loss_fn(z, z_pred)
        proj_loss /= len(zs)
    ...
    def optimize_generator_image(self):
        ...
        with ctx:
            ...
            # logger.info(f"is finite zs_pred: {torch.isfinite(zs_pred[0]).all()}")
            # logger.info(f"is finite zs: {torch.isfinite(zs[0]).all()}")
            self.G_pred = x
            ...
            if not getattr(self.config, "use_dists", False):
                ...
                loss_dists = self.metric_dist(((1 + x) / 2).clamp(0, 1), ((1 + self.batch_inputs.gt) / 2).clamp(0, 1)).mean() * self.config.lambda_dists
            ...
            loss_G = loss_l2 + loss_lpips + loss_dists + loss_disc + proj_loss + loss_edge + loss_tv + loss_l1
            self.accelerator.backward(loss_G)
            if self.accelerator.sync_gradients:
        ...
        # Log something
        loss_dict = dict(G_total=loss_G, G_mse=loss_l2, G_l1=loss_l1, G_lpips=loss_lpips, G_disc=loss_disc, G_repa=proj_loss, G_edge=loss_edge)
    ...
    def optimize_discriminator(self):
        ...
        with ctx:
            ...
            if self.accelerator.sync_gradients:
                grad_norm = self.accelerator.clip_grad_norm_(self.D_params, self.config.max_grad_norm)
            self.D_opt.step()
    ...
    def run(self):
        ...
        self.batch_count = 0
        val_interval = getattr(self.config, "validation_steps", 5000)
        ...
        while self.global_step < self.config.max_train_steps:
            ...
            for batch in self.dataloader:
                ...
                # warmup_steps = getattr(self.config, "warmup_steps", 5000)
                # is_warmup = self.global_step < warmup_steps
                is_warmup = False
                if self.config.use_D and not is_warmup:
                ...
                if generator_step:
                    if self.config.use_vae:
                        loss_dict, grad_norm = self.optimize_generator_image()
                    else:
                        loss_dict = self.optimize_generator_latent()
                else:
                    loss_dict, grad_norm = self.optimize_discriminator()
                ...
                if self.accelerator.sync_gradients:
                    should_advance = (not self.config.use_D and generator_step) or (self.config.use_D and not generator_step) or (self.config.use_D and generator_step and is_warmup)
                    if should_advance:
                        self.global_step += 1
                        self.pbar.update(1)
                        log_dict = {}
                        ...
                        if self.config.log_grads and (self.global_step % self.config.log_grad_steps == 0 or self.global_step == 1):
                            # self.log_grads()
                            if generator_step:
                                log_dict[f"grad_norm/G"] = grad_norm.item()
                            else:
                                log_dict[f"grad_norm/D"] = grad_norm.item()
                        if self.global_step % self.config.checkpointing_steps == 0 or self.global_step == 1:
                            self.save_checkpoint()
                        # if self.global_step % val_interval == 0 and self.global_step > 0:
