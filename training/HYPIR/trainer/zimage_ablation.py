...
from HYPIR.trainer.base_ablation import BaseTrainer, BatchInput
...
class ZImageAblationTrainer(BaseTrainer):
        ...
        if getattr(self.config, "use_qwen", False):
            ...
            with torch.no_grad():
                if self.config.use_txt:
                    ...
                    pos_promt_emb = torch.load(f"debug_inputs/prompt_embeds.pt")
                    # null_prompt_emb = torch.zeros_like(pos_promt_emb)
                    self.c_txt["text_embeds"] = [pos_promt_emb.to(self.device)] * bs
                    # self.c_txt["text_embeds"] = [null_prompt_emb.to(self.device)] * bs
