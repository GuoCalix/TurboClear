...
from hyimage.models.hunyuan.modules.hunyuanimage_dit_repa import HYImageDiffusionREPATransformer
...
def preprocess_raw_image(x, enc_type):
    ...
    if 'clip' in enc_type:
        x = x / 255.
        x = torch.nn.functional.interpolate(x, 224 * (resolution // 256), mode='bicubic', antialias=True)
...
class HunyuanImage21REPATrainer(BaseTrainer):
    ...
    def prepare_batch_inputs(self, batch):
        ...
        if isinstance(batch, dict):
            ...
            gt = batch['gt'].to(self.device, dtype=torch.bfloat16, non_blocking=True)
        else:
            ...
            gt = torch.stack([b['gt'] for b in batch], dim=0).to(self.device, dtype=torch.bfloat16, non_blocking=True)
    ...
    def init_generator(self):
        dit_config = HUNYUANIMAGE_V2_1_DIT_REPA()
        ...
        # 保持在 CPU，设备交给 accelerator.prepare
        self.G = instantiate(dit_config.model, dtype=self.weight_dtype, device="cpu")

        # 加载基础权重；REPA 可能有新增模块，strict=False
        load_hunyuan_dit_state_dict(self.G, dit_config.load_from, strict=False)

        assert isinstance(self.G, HYImageDiffusionREPATransformer), \
            f"Expected REPA, got {type(self.G)}"

        print(f"Type of Generator after load: {type(self.G)}")
    ...
    def _denoise_step(self, latents, timesteps, text_emb, text_mask, byt5_emb, byt5_mask, timesteps_r=None):
        ...
        noise_pred, zs_pred = output[0], output[1]
    ...
    def forward_generator(self):
        ...
        # print(f"----------------Print type(zs_pred): {type(zs_pred)}------------------")
