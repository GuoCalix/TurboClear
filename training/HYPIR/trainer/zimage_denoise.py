...
from HYPIR.trainer.base_denoise import BaseTrainer, BatchInput
from HYPIR.utils.others import NoOpContext, EdgeDetectionModel, total_variation_loss
from HYPIR.model.dvt.models.online_denoiser import Denoiser
...
class ZImageDenoiseTrainer(BaseTrainer):
    def step(self, latents, noise_pred, sigmas, step_i):
        wdtype = self.weight_dtype
        return latents.to(wdtype) - (sigmas[step_i] - sigmas[step_i + 1]).to(wdtype) * noise_pred.to(wdtype)  
    ...
    def prepare_batch_inputs(self, batch, transform=None):
        ...
        if getattr(self.config, "use_qwen", False):
            qwen_visual_embeds, qwen_text_embeds = self.extract_qwen_feature(origin_lq, ["Describe this image in detail"] * bs)
            ...
            with torch.no_grad():
                if self.config.use_txt:
                    ...
                    self.c_txt["text_embeds"] = prompt_embeds,
                else:
                    pos_promt_emb = torch.load(f"debug_inputs/prompt_embeds.pt")
                    self.c_txt["text_embeds"] = [pos_promt_emb.to(self.device)] * bs
    ...
    def init_models(self):
        ...
        # if getattr(self.config, "use_denoiser", True):
        #     self.init_denoiser()
        try:
    ...
    def init_denoiser(self):
        self.denoiser = Denoiser(
            noise_map_height=getattr(self.config, "denoiser_h", 64),
            noise_map_width=getattr(self.config, "denoiser_w", 64),
            feat_dim=getattr(self.config, "denoiser_feat_dim", 3840),
            vit=None,               # keep None to denoise DiT feature maps directly
            enable_pe=True,
            num_blocks=getattr(self.config, "denoiser_blocks", 2),
        ).to(self.device, dtype=self.weight_dtype)
        self.denoiser.train().requires_grad_(True)
        # <--- Weight Initialization Strategy --->
        def _init_weights(m):
            if isinstance(m, (torch.nn.Linear, torch.nn.Conv2d, torch.nn.Conv1d)):
                # 隐藏层使用 Xavier Uniform (适合 Sigmoid/Tanh) 或 Kaiming (适合 ReLU/GELU)
                # 既然是 Transformer 结构，Xavier Uniform 是比较通用的选择
                torch.nn.init.xavier_uniform_(m.weight)
                if m.bias is not None:
                    torch.nn.init.zeros_(m.bias)
        
        # 1. 先对所有层进行标准初始化
        self.denoiser.apply(_init_weights)

        # 2. 寻找最后一层并进行“零初始化”
        # 这里使用启发式方法找到最后一个有参数的层
        last_layer = None
        ...
        if last_layer is not None:
            logger.info("Applying Zero-Initialization to Denoiser output layer.")
    ...
    def init_generator(self):
        ...
        if target_patterns:
            ...
            self._restrict_lora_to_last_blocks(self.G, last_k=2)
    ...
    def _restrict_lora_to_last_blocks(self, model, last_k: int = 2):
        unwrapped = model
        if hasattr(unwrapped, "get_base_model"):
            unwrapped = unwrapped.get_base_model()
        if hasattr(unwrapped, "module"):
            unwrapped = unwrapped.module
        num_layers = len(getattr(unwrapped, "layers", []))
        for name, p in model.named_parameters():
            if "lora_" not in name:
                p.requires_grad = False
                continue
            if "all_final_layer" in name:
                p.requires_grad = True
                continue
            allowed = False
            for idx in range(num_layers - last_k, num_layers):
                if f"layers.{idx}." in name:
                    allowed = True
                    break
            p.requires_grad = allowed
        n_train = sum(p.numel() for p in model.parameters() if p.requires_grad)
        logger.info(f"LoRA trainable params (last {last_k} blocks + final_layer): {n_train}")
    ...
    def extract_qwen_feature(self, lq, prompts):
        ...
        for i in range(batch_size):
            ...
            text_embeds_list.append(text_tokens)
            ...
            if self.accelerator.is_main_process:
                ...
                for i, model in enumerate(models):
                    ...
                    if hasattr(self, "projector") and unwrapped is self.unwrap_model(self.projector):
                        ...
                        weights.pop(i-1)
                    elif hasattr(self, "denoiser") and unwrapped is self.unwrap_model(self.denoiser):
                        torch.save(unwrapped.state_dict(), os.path.join(output_dir, "denoiser.pth"))
                        weights.pop(i-2)
        ...
        def load_model_hook(models, input_dir):
            ...
            for i, model in enumerate(models):
                ...
                if hasattr(self, "projector") and unwrapped is self.unwrap_model(self.projector):
                    ...
                    models.pop(i)
                    proj_path = os.path.join(input_dir, "denoiser.pth")
                    if os.path.exists(proj_path):
                        logger.info(f"Loading projector from {proj_path}")
                        state_dict = torch.load(proj_path, map_location="cpu")
                        unwrapped.load_state_dict(state_dict)
    ...
    def forward_generator(self):
        t_expand = ((1000 - self.batch_inputs.timesteps) / 1000).to(dtype=self.weight_dtype)
        sigmas = torch.tensor([self.config.coeff_t / 1000.0, 0]).to(dtype=self.weight_dtype, device=self.device)
        latent_model_input = self.batch_inputs.z_lq
        # last_feats = self._denoise_step(
        #     latent_model_input, t_expand, 
        #     self.c_txt["text_embeds"],
        #     timesteps_r=None
        # )
        # last_feats = last_feats.to(self.weight_dtype)
        # B, N, C = last_feats.shape
        # H = W = 64
        # pH = pW = 2
        # noise_pred = self.denoiser(last_feats, t_expand)
        # noise_pred = noise_pred.view(B, H, W, pH, pW, 16).permute(0, 5, 1, 3, 2, 4).reshape(B, 16, H*pH, W*pW)
