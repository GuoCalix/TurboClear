...
try:
    ...
    def get_unsafe_globals_in_checkpoint(path):
        return []
...
class BaseTrainer:
    ...
    def init_environment(self):
        ...
        accelerator_mixed_precision = getattr(
            self.config,
        ...
        if accelerator.is_main_process:
            tracker_project_name = getattr(self.config, "wandb_project", "train")
            tracker_init_kwargs = {}
            wandb_run_name = getattr(self.config, "wandb_run_name", None)
        ...
        weight_dtype_name = str(getattr(self.config, "weight_dtype", accelerator.mixed_precision)).lower()
        weight_dtype = torch.float32
        if weight_dtype_name in ("fp16", "float16"):
            weight_dtype = torch.float16
        elif weight_dtype_name in ("bf16", "bfloat16"):
    ...
    def load_accelerator_state(self, path):
        # Optimizer checkpoints are trusted local training state and may contain
        # non-tensor Python objects (for example OmegaConf ListConfig).
        try:
            self.accelerator.load_state(path, load_kwargs={"weights_only": False})
        except TypeError as exc:
            if "load_kwargs" not in str(exc) and "weights_only" not in str(exc):
                raise
            logger.warning(
                "Accelerate/torch does not accept load_kwargs={'weights_only': False}; "
                "falling back to the default load_state path."
            )
    ...
    def log_metrics_csv(self, log_dict, step, filename="train_metrics.csv"):
        ...
        existing_rows = None
        fieldnames = ["step"] + sorted(key for key in row.keys() if key != "step")
        if os.path.exists(csv_path):
            with open(csv_path, "r", newline="", encoding="utf-8") as f:
                ...
                new_keys = [key for key in sorted(row.keys()) if key not in fieldnames]
                if new_keys:
                    existing_rows = list(reader)
                    fieldnames.extend(new_keys)

        write_mode = "w" if existing_rows is not None or not os.path.exists(csv_path) else "a"
